...
# Hyperparams
hparams = { "loss_type": "mean squared error", "batch_size": 64, "num_epochs": 10, "beta_entropy": 0.01, "lr": 0.0003, "num_episodes_between_training": 20, "discount_factor": 0.999 }
with open("hyper.env", 'r') as hf:
    rawtext = hf.readline()
    while len(rawtext) > 0:
        rawtext = rawtext.replace('\n', '')
        keyval = rawtext.split(':')
        hparams[keyval[0]] = type(hparams[keyval[0]])(keyval[1])
        rawtext = hf.readline()
...
job_duration_in_seconds = int(os.environ.get("TRAINING_TIME", "60")) * 60
print("Max run in minutes", job_duration_in_seconds / 60)
...
estimator = RLEstimator(entry_point="training_worker.py",
                        ...
                        train_max_run=int(job_duration_in_seconds), # Maximum runtime in seconds
                        hyperparameters={"s3_bucket": s3_bucket,
                                         "s3_prefix": s3_prefix,
                                         "aws_region": aws_region,
                                         "model_metadata_s3_key": "s3://{}/params/model_metadata.json".format(s3_bucket),
                                         ...
                                         "beta_entropy": hparams["beta_entropy"],
                                         "lr": hparams["lr"],
                                         "num_episodes_between_training": hparams["num_episodes_between_training"],
